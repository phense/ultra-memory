...
| `ATOMIC_GRADUATE_CLUSTER_COS` | float | `0.80` | Candidate-clustering cosine. `atomic_graduate` clusters the pending candidates **together with existing-page `## Signal` seed vectors** at this threshold so each incident graduates once: a cluster containing an existing page MERGEs all its candidates into it (uncapped), a seedless cluster CREATEs one atomic from the longest-body representative (capped). Deliberately *below* the page-merge `dedup_upper` (0.84) — merging is additive/safe, so a slightly looser group is low-risk. Calibrated from the live pilot. No embedder → falls back to the per-candidate gate. |
