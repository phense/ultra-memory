...
| `_time.py` | One source of truth for the canonical Zulu wire format (`now_utc_zulu` / `hours_between` / `ZULU_FMT`); stdlib-only, import-safe everywhere. Distinct from the `.isoformat()` call sites, which keep a deliberately different format. |
...
| `unified_query.py` | **(SP-3)** the warm cross-store retrieval surface: `unified_recall` (memory cosine + generic knowledge BM25/cosine, FU-4 RRF, × outcome_weight) + the fail-closed `topic_scope_from_env`. No LLM. The final fused sort uses a **deterministic total order** `(-weighted_score, (kind, key))` so a tie cannot reorder run-to-run under `PYTHONHASHSEED` (the rrf dict is built from a `set`); the secondary key changes only tie-order, never which score ranks where. **Two PERF-only fixes (R4 bug-hunt — results byte-identical):** (1) the knowledge-side embedding fetch is **batched** — one `… target_id IN (…)` query per `_EMBED_FETCH_CHUNK` (=500) slugs instead of the old per-slug N+1 (`cached` is rebuilt in `by_slug` order so the cosine tie-order is unchanged); (2) `_bm25_rank` **memoizes** the query-independent corpus tokenization (`tokenized`/lengths/avgdl) on a stable **sha1 content fingerprint** of the docs (`_bm25_corpus_stats`, single-entry cache) — an unchanged corpus is tokenized once, any doc edit/add/remove changes the fingerprint and recomputes (never stale). The fingerprint is sha1, NOT `PYTHONHASHSEED`-salted `hash()`, so the cache key is process-stable; the df + scoring still run per call. |
...
| `maintenance/session_ingest.py` | **(Subsystem 4)** the session-as-ingestion-stream beat: a throttled OAuth pass mines each finished session's transcript digest (tool-output-free, redacted) for THREE outputs in ONE call — durable knowledge → memories, user corrections → `feedback`, and **skill-tagged learnings** (for the tracked skills that session used, read from its `skill_learning_candidate` markers and grounded to that set) → `node_type='learning', index_hook=<skill>, created_by='background_review'` (the SP-10 / `Learnings.md` substrate). It then resolves those candidate markers, **superseding the thin consolidate feeder**. Runs FIRST in the pipeline (before consolidate); ships-disabled (`SESSION_INGEST_ENABLE`). |
